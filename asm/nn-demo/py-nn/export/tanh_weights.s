* tanh_weights.s -- Q16.16 weights for the tanh+softmax digit MLP
* Architecture: 256 (16x16) -> 16 -> 10   (hidden = tanh, output = softmax)
* All words are signed Q16.16 (value = word / 65536).
* Memory order (read sequentially by the inference loop):
*   W1DATA : 16*256 words, row-major  W1[h][i]  (h outer, i inner)
*   B1DATA : 16 words   b1[h]
*   W2DATA : 10*16 words, row-major  W2[k][h]  (k outer, h inner)
*   B2DATA : 10 words  b2[k]
*
W1DATA:  dc 0x00000203    * W1[0][0]
         dc 0xFFFFFDE3
         dc 0x00000A40
         dc 0x0000173E
         dc 0xFFFFA9C6
